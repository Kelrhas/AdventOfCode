...
{
	auto operator()(const Face &f) const
	{
...
void Solve()
{
	...
	for (const auto &f : faces)
	{
		...
		while (!visitPending.empty() && !bOutside)
		{
			...
			visitPending.pop();
			if (coord.x - 1 >=	shellCube.minX)	TryAddVisit(coord - glm::ivec3{ 1, 0, 0 }); else bOutside = true;
			if (coord.x + 1 <	shellCube.maxX)	TryAddVisit(coord + glm::ivec3{ 1, 0, 0 }); else bOutside = true;
			if (coord.y - 1 >=	shellCube.minY)	TryAddVisit(coord - glm::ivec3{ 0, 1, 0 }); else bOutside = true;
			if (coord.y + 1 <	shellCube.maxY)	TryAddVisit(coord + glm::ivec3{ 0, 1, 0 }); else bOutside = true;
			if (coord.z - 1 >=	shellCube.minZ)	TryAddVisit(coord - glm::ivec3{ 0, 0, 1 }); else bOutside = true;
			if (coord.z + 1 <	shellCube.maxZ)	TryAddVisit(coord + glm::ivec3{ 0, 0, 1 }); else bOutside = true;
		}
