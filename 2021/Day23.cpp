...
template<>
struct std::hash<Day23::Amphipod>
{
	auto operator()(const Day23::Amphipod &a) const
	{
...
template<>
struct std::less<Day23::State>
{
	bool operator()(const Day23::State &lhs, const Day23::State &rhs) const
	{
		return (Day23::cacheDist[lhs] + EstimateRemainingEnergy(lhs)) > (Day23::cacheDist[rhs] + EstimateRemainingEnergy(rhs));
	}
};
...
namespace Day23
{
	...
	void Display(const State &_state)
	{
		...
		for (int y = 1; y <= ROOM_DEPTH; ++y)
		{
			...
			for (int i = 2; i <= 8; i += 2)
			{
				if (auto it = find_if(_state.begin(), _state.end(), [i, y](const Amphipod &pod) { return pod.pos.x == i && pod.pos.y == y; });
					it != _state.end())
				{
					SetConsoleTextAttribute(hConsole, it->bGoodPlace ? CONSOLE_GREEN : CONSOLE_WHITE);
					cout << (char)(it->eType + 'A');
